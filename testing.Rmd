...
ccomp_6_SumShot <- ccomp_5_summary %>% 
  ...
  select(ID,Site,daylight,Depth_Stratum,Deployment,ACat,Taxa_Groups,Acoustic_Category, Taxa_updated,CAAB_updated,
         nos,weight_g,
         
         Weight_Std_gm3_gps = Weight_Std_gm3_gps, #select gps or speed
         Abd_Std_nosm3_gps = Weight_Std_gm3_gps, #select gps or speed
         AvgL_mm,
...
ccomp_Ind_Taxa_M_noDep <- ccomp_Ind_Taxa_M %>% 
  group_by(Site,daylight,Depth_Stratum, Taxa_updated) %>% 
    summarise(Nos = sum(Nos), Wtgm3 = sum(Weight_g),Anosm3 = sum(Abd_nosm3),Wtgm3_avg = mean(Weight_gm3),Anosm3_avg = mean(Abd_nosm3))
...
#Acoustic_Category - G_nGB
TW_std_ID_Taxa <- ccomp_Acoustic_Category%>% 
  group_by(Site,daylight,Depth_Stratum,Deployment) %>% 
...
unique(ccomp_Acoustic_Category$Acoustic_Category)
``` 
