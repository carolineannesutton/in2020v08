...
```{r select individual taxa}

TW_std_ID_Taxa <- ccomp_3 %>% 
  group_by(Site, daylight,Depth_Stratum,Deployment,ACat,Taxa) %>% 
  filter(ACat=="Fish") %>% 
 # filter(Taxa =="Lampanyctus australis") %>% 
  ...
  mutate(TWgm3 = ifelse(daylight %in% "Night", TWgm3*-1, TWgm3)) %>% 
  group_by(Site, daylight,Depth_Stratum) %>% 
  summarise(TWgm3_avg = mean(TWgm3))
