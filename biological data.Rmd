...
taxa <- distinct(ccomp_1,Taxa) %>% 
  arrange(Taxa)

# modificaitons required
# fish >  Fish
# sp > spp
# see code below and move to here
...
# Remainder > remainder
# Sepiidae - undifferentiated   <  Sepiolidae - undifferentiated


taxa2 <- filter(ccomp_1, is.na(CAAB)) %>% 
  distinct(Taxa)

filter(ccomp, Taxa == "Aosellota")
```
...
ccomp_2 <- ccomp_1 %>% 
  mutate(TaxaVed = ifelse(Taxa =="Aosellota","Asellota",Taxa)) %>%
  ...
  mutate(ACat = ifelse(TaxaVed =="Larval Fish", "Fish",
                ifelse(TaxaVed =="Larval fish", "Fish",
