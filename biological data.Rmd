...
ccomp_1 <- ccomp %>% 
  ...
  mutate(TaxaVed = ifelse(is.na(Taxa_ed), Taxa, Taxa_ed)) %>% 
  mutate(CAABved = ifelse(is.na(CAAB_ed), CAAB,CAAB_ed)) 


# test for duplicates in taxa
taxa <- distinct(ccomp_1,TaxaVed) %>% 
  arrange(TaxaVed)
...
filter(ccomp, Taxa == "Krefftichthys anderssoni")
...
ccomp_2 <- ccomp_1 %>% 
  #mutate(TaxaVed = ifelse(Taxa =="Aosellota","Suborder Asellota - undifferentiated",Taxa)) %>%
  #mutate(TaxaVed = ifelse(Taxa =="Krefftichthys anderssoni",37122754,CAAB)) %>% 
  ...
  mutate(ACat = ifelse(TaxaVed =="Larval Fish", "Fish",
                ifelse(TaxaVed =="Fish", "Fish",
                ifelse(TaxaVed =="Eel Larvae clear", "Fish",
                ifelse(TaxaVed =="Bathylagus tenuis", "Fish",
                ifelse(TaxaVed =="Iniea sp.", "Amphipods",
                ifelse(TaxaVed =="Mixed Prawn larvae", "Prawn",
                ifelse(TaxaVed ==" Aosellota", "Isopods",
                ifelse(TaxaVed =="Red Gelatinous", "Siphonophore",
                ifelse(TaxaVed =="Pelagic worm", "Other",  
                ifelse(TaxaVed =="Remainder", "Tunicates",  
                ifelse(TaxaVed =="Spherical eggs", "Other",  
                ifelse(TaxaVed =="Hyperia medusarium", "Amphipods",     
                ifelse(TaxaVed =="Aphicaryon acaule", "Siphonophore",      
                ifelse(TaxaVed =="Salpa thompsoni", "Tunicates",   
                ifelse(TaxaVed =="Hydromedusa", "Medusa",    
                ifelse(TaxaVed =="Spongiobranchaea australis", "Gymnosome",
                ifelse(TaxaVed =="Eusirella spp", "Copepod",   
                ifelse(TaxaVed =="Asellota", "Isopods",  
                ifelse(TaxaVed =="Phyllosoma", "Crustacean",  
                ifelse(TaxaVed =="Senticaudata", "worms",
                ifelse(TaxaVed =="remainder", "Tunicates",      
                ifelse(TaxaVed =="eggs", "Other",  
                ifelse(TaxaVed =="Phronima houses", "Other",
                ifelse(TaxaVed =="Salpa thompsoni - aggregate", "Tunicates", 
                ifelse(TaxaVed =="Salpa thompsoni - solitary juv", "Tunicates", 
                ifelse(TaxaVed =="Salpa thompsoni - solitary", "Tunicates", 
                       ACat))))))))))))))))))))))))))))) %>% 
...
length_1 <- length %>% 
  rename(Acc = 'Acc #') %>% 
  left_join(ccomp_5, by = c("Deployment", "Acc")) %>% 
...
```{r catch summary}
ccomp_6 <- ccomp_5 %>% 
  dplyr::group_by(Deployment,ACat,Taxa_Groups,Taxa_updated,CAAB_updated) %>% 
...
write.csv(ccomp_7,"Results/IN2020_V08_CatchComp.csv")

Species_by_Site <- ccomp_7 %>% 
  group_by(Site) %>% 
  distinct(Taxa_updated, .keep_all = TRUE)
...
TaxaSots <- ggplot(Taxa_TW %>% 
  ...
  theme(legend.position = "none")+
  facet_grid(.~daylight)+
  scale_fill_manual(values=c("purple","grey","orange","dark blue","dark green","light pink","light blue","yellow"))+
