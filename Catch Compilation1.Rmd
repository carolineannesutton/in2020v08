...
Bladder_update <- read_csv("Results/Solace_Species with Bladder status3.csv")
```
...
ccomp_2 <- ccomp_1 %>% 
 dplyr::rename(Deployment = `Operation No`, Acc = `Accession No`, Sample_Type = `Sample Type`) %>%
...
ACatDATA_bladder <- ACatDATA %>% 
  ...
  distinct(ACatDATA_bladder,ACat)


# including updated Fish bladder data
  Bladder_update <- Bladder_update %>% 
    select(Taxa_updated, bladder_research)
...
ccomp_3_ACATDB <- left_join(ccomp_3, ACatDATA_bladder, by = c("Taxa_updated"= "Species_Name")) %>% 
  ...
  mutate(ACat = ifelse(ACat == "FSB", "FGB", ACat)) %>% 
  select(-ACat.y, - ACat.x)
  

# adding updated fish bladder data
  #left_join(Bladder_update, by = c("Taxa_updated" = "Taxa_updated")) 
  ccomp_3_ACATDB1 <-  left_join(ccomp_3_ACATDB, Bladder_update, by = "Taxa_updated") %>% 
  mutate(ACat = ifelse(is.na(bladder_research), ACat,bladder_research))

distinct(ccomp_3_ACATDB1,ACat)
x <-filter(ccomp_3_ACATDB1, ACat =="F")
```
...
ccomp_3_ACATDB_TGroups <- ccomp_3_ACATDB1 %>% 
  mutate(Taxa_Groups = 
