...
select(Acoustic_Category,Taxa_Groups,ACat,Taxa_PLAOS) %>% 
  arrange(Acoustic_Category) %>% 
    distinct(Acoustic_Category,ACat,Taxa_Groups,Taxa_PLAOS)
...
SOLACE_SpeciesList <- SOLACE %>% 
  select(Acoustic_Category,Taxa_Groups,ACat,CAAB_updated,Taxa_updated) %>% 
  distinct(Taxa_updated,.keep_all = TRUE) 

write.csv(SOLACE_SpeciesList,"Results/Solace_SpeciesList.csv")
```
...
write.csv(SOLACE_1,"figures/Paper/CumPercent.csv")
# 
 SOLACE %>% filter(Acoustic_Category == "FnGB")
```
