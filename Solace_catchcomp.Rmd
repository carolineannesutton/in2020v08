...
library(janitor)
# read in data
CC <- read_csv("data/in2020_v08_samples_validation_220810.csv", col_types = list(Acc_ed = col_character()))
...
CC <- CC %>% 
  ...
  select(Voyage, Deployment,Acc,Acc_ed,Acc_edold,Sample_Type,Taxa,CAAB, Taxa_ed,CAAB_ed, weight_g =`Weight (g)`, Count,Comments,Storage_temp =`Storage temperature`,Preservative,`Container name`,Taxa_updated, CAAB_updated) %>% 
  mutate(Acc_ed = as.numeric(Acc_ed)) %>% 
...
species <- filter(CC_ACATDB_TGroups,Taxa_updated =="Pyrosoma atlanticum") 
```
...
unique(paste0(CC_ACATDB_Acoustic_Category$Acoustic_Category," ",CC_ACATDB_Acoustic_Category$ACat))

check <- select(CC_ACATDB_Acoustic_Category,Acoustic_Category,CAAB,Taxa_updated)

siph <- filter(CC_ACATDB_Acoustic_Category,ACat =="Siphonophore") %>% 
  distinct(Taxa_updated)

# Table of Taxa Groupings
Taxa_Groupings <- CC_ACATDB_Acoustic_Category %>% 
...
CC_nosplits <- CC_ACATDB_Acoustic_Category %>%
...
ggplot(filter(Shot_withgps,Depth_Stratum == "Lmeso" |Depth_Stratum=="Meso" | Depth_Stratum == "Epi"), 
       aes(x=Deployment,colour = Depth_Stratum)) + 
  geom_point(aes(y=VolumeFiltered_m3_speed, shape="V_speed")) + 
  geom_point(aes(y=VolumeFiltered_m3_gps, shape="V_gps")) 

Shot_withgps <- Shot_withgps %>% 
  mutate(vdiff = abs(Vdiff_sp_gps))

ggplot(filter(Shot_withgps,Depth_Stratum == "Lmeso" |Depth_Stratum=="Meso" | Depth_Stratum == "Epi"), 
       aes(x = Deployment, y = vdiff, colour = Depth_Stratum))+
  geom_point()+
  scale_x_continuous(breaks = seq(min(Shot_withgps$Deployment),
                                  max(Shot_withgps$Deployment),
                                  by = 2))

```
## fixing the human error wrt to the distance estimated by ship's position. 
Dave Watts examined the changes in ships speed to estimate the ships position at the beginning and end of suspect tows and made a best estmiate for distance towed. 

tows with the biggest difference b/w distance towed estimates by ships position and speed:

deployments:
29 best distance estimate = 3284.63
30 best distance estimate = 1593.13
42 best distance estimate = 2639.09
6  best distance estimate = 6123.23
18 best distance estimate = 1525.95
28 best distance estimate = 5048.55
33  best distance estimate = 825.94
25  best distance estimate = 1537
1  best distance estimate = 3613.26

```{r}
Distance_towed_correction <- Shot_withgps %>% 
  select(Deployment,date_start,lat_decdeg_start, long_decdeg_start,towtime,vdiff,distance_m_byspeed)
```
...
CC_forlengthjoin <- select(CC_nosplits, Deployment, Acc, Acc_ed,Taxa_updated, CAAB_updated)

length <- length %>% 
# joining length data to the catch comp and site data for graphing
  left_join(CC_forlengthjoin, by = c("Deployment", "Acc")) %>% 
...
length_Dups <-length[duplicated(length$ID), ]

# choose the shots where discrete depths were sampled

unique(length$Depth_Stratum)

# filter length data for depth stratified sites only
#plot length data
  length_sites <-  length %>% 
    select(Site, daylight, Depth_Stratum, 
    Deployment, Acc, Acc_ed, Species,Taxa_updated, 
    `Length _mm`) %>% 
    # filter for depth stratified shots
   filter(Depth_Stratum == "Meso" |
            Depth_Stratum == "Epi"| 
            Depth_Stratum =="Lmeso") %>%   
    # filter for micronekton 
  filter(`Length _mm`< 5500)
  
write.csv(length_sites,"Results/IN2020_V08_Lengths.csv")
```

## add in the average length data to the catch comp
```{r adding in length data}
  
CC_nosplits_length <- left_join(CC_nosplits, length_avg, by = c("Deployment", "Acc_ed")) 
...
###################### need to work out how to summarise different length for sample species ################################################################
...
## must average the lengths here b/c sometimes there was two length classes for the same species group. 
# But: 'Salpa thompsoni solitary' and 'Salpa thompsoni aggregate' will remain separate

### this has resolved the duplicate problem

CC_summary <- CC_nosplits_length %>% 
  group_by(Deployment,ACat,Taxa_Groups,Acoustic_Category,Taxa_updated,CAAB_updated) %>% 
  summarise(nos= sum(Count, na.rm = TRUE),
            weight_g = sum(weight_g, na.rm = TRUE), 
            AvgL_mm = mean(AvgL_mm, na.rm = TRUE),
            n()) %>% 
  rowid_to_column("ID")
#ccomp_AEC <- ccomp_1 %>% 
...
## Standardising the catch comp with Volume Filtered
```{r Standardising catch data }
CC_summary_shot <- CC_summary %>% 
  left_join(Shot_withgps, by="Deployment") %>% 
  ...
  #making sure duplicate rows are ignored
  distinct(ID,.keep_all = TRUE)

# write data
write.csv(CC_summary_shot,"Results/IN2020_V08_Standardised_withLengths.csv")
